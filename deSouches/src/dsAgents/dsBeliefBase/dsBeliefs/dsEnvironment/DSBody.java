...
public class DSBody {
  ...
  public boolean isCellAt(Point position) {
    for (DSCell cell : PBody)
      if ((cell.getX() == position.x) && (cell.getY() == position.y)) return (true);
    return (false);
  }

  public int getCellType(Point position) {
    for (DSCell cell : PBody)
      if ((cell.getX() == position.x) && (cell.getY() == position.y)) return (cell.getType());
  ...
  public LinkedList<String> getAllDirectionsAttached() {
    ...
    if (isCellAt(new Point(0, 1))) directions.add("s");
    return (directions);
  }
  ...
  boolean pointInBody(DSBody body, Point point) {
    for (DSCell cell2 : body.getBodyList())
      if ((point.x == cell2.getX()) && (point.y == cell2.getY())) return (true);
    return (false);
  }

  public boolean matchBody(DSBody body) {
    if (PBody.size() != body.getBodyList().size()) return (false);

    for (DSCell cell : PBody) {
      if (!pointInBody(body, cell.getPosition())) return (false);
    }
    return (true);
  }
  ...
  DSCell rotateCW(DSCell cl) {
    int sx, sy;
    if (cl.getX() == 0) return (new DSCell(-cl.getY(), 0, cl.getType(), cl.getTimestamp()));
    if (cl.getY() == 0) return (new DSCell(0, cl.getX(), cl.getType(), cl.getTimestamp()));

    sx = Integer.signum(cl.getY()) * -1;
    sy = Integer.signum(cl.getX());

    return (new DSCell(
        Math.abs(cl.getY()) * sx, Math.abs(cl.getX()) * sy, cl.getType(), cl.getTimestamp()));
  }
  ...
  public void rotateBody(boolean cw) { // true cw, false ccw
    ...
    DSCell cl2;
    for (DSCell cl : PBody) {
      if (cw) cl2 = rotateCW(cl);
      else cl2 = rotateCCW(cl);
      newBody.add(cl2);
    }
    PBody = newBody;
  }
  ...
  public DSBody cloneBody() {
    ...
    for (DSCell cell : PBody) if ((cell.getX() != 0) || (cell.getY() != 0)) newBody.addCell(cell);
    return (newBody);
  }
  ...
  public void removePart(Point position) {
    LinkedList<DSCell> newBody = new LinkedList<DSCell>();
    for (DSCell cell : PBody)
      if ((cell.getX() != position.x) || (cell.getY() != position.y)) newBody.add(cell);
    PBody = newBody;
  }
