...
public class DSGetBlock extends DSGoal {
    ...
    // pokud depot vi, snazi se k nemu dojit, sestavi cestu k nemu, pokud by nahodou spadl, sestavuje cestu znovu
    ...
    DSPlan makePlanGetFromDispenser(DSAgent agent){
        ...
        if (PDispenserLocation == null)
            PDispenserLocation = agent.getNearestDispenser(PBlockType);

        if (PDispenserLocation != null) {  // nevidi dispenser

            //      PPlan = new DSAStar().computePath(agent.getMap(), agent.getMap().getAgentPos(), PDestination, agent.getBody(),300, agent);
            ...
            HorseRider.inform(TAG, "makePlan: " + agent.getEntityName() + " pronasleduje dispenser na " + PDispenserLocation);
