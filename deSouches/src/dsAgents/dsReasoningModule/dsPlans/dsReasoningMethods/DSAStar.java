    ...
    private static void printList(String agentName, LinkedList<DSAStarItem> lst) {
        ...
        for (DSAStarItem item : lst) {
            if(item.PAction!=null) {
                message.append("[").append(item.PAction.actionText()).append(" ; ").append("/").append(item.getPosition().x).append(",").append(item.getPosition().y).append("/:").append(item.getCost()).append("/").append(item.getHeuristic()).append("] ");
            }
        }
        System.out.println("printList: "+message);
    }

    private static void printStructures(String agentName, LinkedList<DSAStarItem> open, LinkedList<DSAStarItem> close, int steps) {
...
//        printList(open);
    ...
    private static boolean isInClosed(Point position, DSBody body, LinkedList<DSAStarItem> closed) {
        for (DSAStarItem dsaStarItem : closed) {
            if (dsaStarItem.getPosition().equals(position)) {
                if(dsaStarItem.getBody().matchBody(body))
                    return true;
            }
        }
        return false;
    ...
    /**
     * inserts new value
     * skips if better present, removes old with same cost and sorts list from lowest.
     *
    ...
    private void insertCheapest(LinkedList<DSAStarItem> open, DSAStarItem newItem) {
        ...
        while (i.hasNext() && (!placed || !removed)) {    // iterate through items until placed and removed or ran out of elements
            DSAStarItem item = i.next();
            if (!removed && item.getPosition().equals(newItem.getPosition())) { // same position
                removed = true;
                if ((newItem.getCost()+newItem.getHeuristic()) < (item.getCost()+item.getHeuristic())) {                        // new one is better
                    i.remove();
                } else {                                                          // old one is better
                    placed = true;
                }
            }
            if (!placed && (item.getCost()+item.getHeuristic()) >= (newItem.getCost()+newItem.getHeuristic())) { // only worse values after this
    ...
    private void expandItem(DSMap map, DSAStarItem item, DSBody agentBody, LinkedList<DSAction> actions, Point to,
                                   LinkedList<DSAStarItem> open, LinkedList<DSAStarItem> closed, int step) {

        while (!actions.isEmpty()) {
            DSAction action = actions.pop();
            DSAStarItem newItem = action.simulate(map, item, agentBody,step); // TODO ta nula na konci je provizorka
            if (newItem != null) {
    ...
    private DSAStarItem aStarIterate(DSMap map, LinkedList<DSAStarItem> open, LinkedList<DSAStarItem> closed, Point to,
                                            DSBody finalBody, int steps, DSAgent agent, DSBody agentBody) {
      ...
      //      System.out.println("A* dlja "+agent.getEntityName()+" dava do cosed "+item.PPosition);
            ...
            if (item.getPosition().equals(to))
                if(item.getBody().matchBody(finalBody))
                    return item;
            // expanding actions
    ...
    private DSPlan aStar(String planName, int priority,
                         DSMap map, LinkedList<DSAStarItem> open, LinkedList<DSAStarItem> close, Point to,
                                DSBody finalBody, int steps, DSAgent agent, DSBody agentBody) {
        DSAStarItem goal;
 ...
 //       map.printMap("Astar "+agent.getEntityName());
    ...
    public DSPlan computePath(
            String planName, int priority, DSMap map, Point from, Point to, DSBody finalBody, int steps, DSAgent agent) {
    ...
    //    HorseRider.inform(TAG, "computePath: agent "+agent.getEntityName()+" body "+agent.getBody().bodyToString());
        // posunout na spravnou pozici agentovo telo na mape skupiny
        DSBody agentBodyShifted = DSBody.shiftBody(from, agent.getBody());
     //   HorseRider.inform(TAG, "computePath: agent "+agent.getEntityName()+" body shifted "+agentBodyShifted.bodyToString()+" disp "+agent.getGroup().getDisplacement(agent));;

        open.add(new DSAStarItem(from, null, null, agent.getBody(), 0, heuristic(from, to)));
     //   HorseRider.inform(TAG, "computePath: "+agent.getEntityName()+" final body "+finalBody.bodyToString());
        return (aStar(planName,priority, map, open, close, to, finalBody, steps, agent, agentBodyShifted));
    }
}
