...
public class DSCoallitionReasoning {
  ...
  public static synchronized LinkedList<HashMap<DSAgent, Integer>> proposeTaskCoallition(
      LinkedList<DSAgent> workers,
          ...
          for (Point dispenserLocation : dispensersForType) {
            ...
            System.out.println(
                "Agent "
                    ...
                    + " through "
                    + dispenserLocation.toString()
                    + " to "
                    + goal.toString()
                    + " is "
                    + distance);
            if ((bestDistance == -1) || (distance < bestDistance)) bestDistance = distance;
          }
