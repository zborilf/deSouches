...
public class DSTaskType23 extends DSTaskType {
  ...
  public Point formationPosition(DSAgent agent, Point position) {
    ...
    if (agent == PLeutnant2) return (new Point(position.x, position.y + 4));
    if (agent == PLeutnant3) return (new Point(position.x + 1, position.y + 4));
    return (null);
  }
