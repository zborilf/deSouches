...
public class DSTaskType36 extends DSTaskType {
  ...
  DSPlan makePlanL3() {
    ...
    DSConnect connect =
        new DSConnect(PMaster.getEI(), getConnectDirection(PLeutnant3), PMaster.getEntityName());
    plan.appendAction(connect);
  ...
  public Point blockPosition(int agent) {
    ...
    if (agent == 4) return (new Point(1, 1));
    return (null);
  }
  ...
  public Point formationPosition(DSAgent agent, Point position) {
    ...
    if (agent == PLeutnant1) return (new Point(position.x - 1, position.y + 1));
    if (agent == PLeutnant2) return (new Point(position.x - 2, position.y + 1));
    if (agent == PLeutnant3) return (new Point(position.x + 1, position.y));
    return (null);
  }
