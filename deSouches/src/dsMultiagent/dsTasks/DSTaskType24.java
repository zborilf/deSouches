...
public class DSTaskType24 extends DSTaskType {
  ...
  public Point blockPosition(int agent) {
    ...
    if (agent == 4) return (new Point(1, 3));
    return (null);
  }
  ...
  public Point formationPosition(DSAgent agent, Point position) {
    ...
    if (agent == PLeutnant1) return (new Point(position.x, position.y + 3));
    if (agent == PLeutnant2) return (new Point(position.x + 2, position.y + 2));
    if (agent == PLeutnant3) return (new Point(position.x + 2, position.y + 3));
    return (null);
  }
