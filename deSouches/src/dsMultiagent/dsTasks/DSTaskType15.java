...
public class DSTaskType15 extends DSTaskType {

  @Override
  DSPlan makePlanL1() {
    DSPlan plan = new DSPlan("Leutnant1, task15", 2);
  ...
  @Override
  DSPlan makePlanL2() {
    DSPlan plan = new DSPlan("Leutnant2, task15", 2);
  ...
  @Override
  DSPlan makePlanL3() {
    DSPlan plan = new DSPlan("Leutnant3, task15", 2);
  ...
  public Point blockPosition(int agent) {
    ...
    if (agent == 4) return (new Point(-2, 2));
    return (null);
  }
  ...
  public Point formationPosition(DSAgent agent, Point position) {
    ...
    if (agent == PLeutnant3) return (new Point(position.x - 3, position.y + 2));
    return (null);
  }
  ...
  public DSTaskType15() {
    ...
    PTaskArea.addCell(new DSCell(-3, 2, 0, 0));
  }
}
