...
public class DSTaskType31 extends DSTaskType {

  @Override
  DSPlan makePlanL1() {
    DSPlan plan = new DSPlan("Leutnant1, task18", 2);
    DSConnect connect = new DSConnect(getConnectDirection(PLeutnant1), PLeutnant3.getEntityName());
  ...
  DSPlan makePlanL2() {
    ...
    DSConnect connect = new DSConnect(getConnectDirection(PLeutnant2), PMaster.getEntityName());
    plan.appendAction(connect);
  ...
  DSPlan makePlanL3() {
    ...
    DSConnect connect = new DSConnect(getConnectDirection(PLeutnant3), PLeutnant1.getEntityName());
    plan.appendAction(connect);
  ...
  public Point blockPosition(int agent) {
    ...
    if (agent == 3) return (new Point(1, 1));
    if (agent == 4) return (new Point(-1, 2));
    return (null);
  }
  ...
  public Point formationPosition(DSAgent agent, Point position) {
    ...
    if (agent == PLeutnant2) return (new Point(position.x, position.y + 1));
    if (agent == PLeutnant3) return (new Point(position.x - 1, position.y + 1));
    return (null);
  }
