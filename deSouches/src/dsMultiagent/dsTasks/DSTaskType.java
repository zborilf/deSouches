...
public abstract class DSTaskType {
  ...
  public Point blockPosition(DSAgent agent) {
    ...
    if (agent == PLeutnant3) return (blockPosition(4));
    return (null);
  }
  ...
  public DSPlan dancePlan(DSAgent agent, String taskName) {
    if (agent == PMaster) return (makePlanMaster(taskName));
    if (agent == PLeutnant1) return (makePlanL1());
    if (agent == PLeutnant2) return (makePlanL2());
    if (agent == PLeutnant3) return (makePlanL3());
    return (null);
