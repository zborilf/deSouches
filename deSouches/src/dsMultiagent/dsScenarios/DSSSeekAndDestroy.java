...
public class DSSSeekAndDestroy extends DSScenario {
  ...
  static final int _Seek = 1;
  static final int _Destroy = 2;
  ...
  public void goalCompleted(DSAgent agent, DSGGoal goal) {
    ...
    if (PState == _Seek) {
      PState = _Destroy;
      Point direction =
          PAgent.getMap().objectAroundCell(PAgent.getMapPosition(), DSCell.__DSObstacle);
      if (direction == null) {
        initScenario(PAgent.getStep());
        return;
      }
      Point targetPosition =
          new Point(
              PAgent.getMapPosition().x + direction.x, PAgent.getMapPosition().y + direction.y);
      if (PAgent.getMap().objectAroundCell(targetPosition, DSCell.__DSEntity_Friend) != null)
        initScenario(PAgent.getStep());
      else {
        Point relDirection =
            new Point(
                direction.x - PAgent.getMapPosition().x, direction.y - PAgent.getMapPosition().y);
  ...
  public boolean initScenario(int step) {
    ...
    Point PObstacleAt =
        PAgent.getMap().nearestObject(DSCell.__DSObstacle, PAgent.getGroup().getUGuardedGoalArea());
