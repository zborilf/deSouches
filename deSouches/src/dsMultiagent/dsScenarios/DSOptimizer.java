...
public class DSOptimizer {
    ...
    HashMap<Integer,LinkedList<Point>> getDispensersPositions() {
        HashMap<Integer,LinkedList<Point>> dispensersPositions=new HashMap<Integer,LinkedList<Point>>();
    ...
    DSODispenserGoalMission shortestDistanceAgentDispenser(DSAgent agent, int dispenserType){
        ...
        if(dispenserPositions==null)
            return(null);
        if(dispenserPositions.isEmpty())
            return(null);
        optimalDGM=new DSODispenserGoalMission(agent,dispenserPositions.getFirst(),PGoalPosition, dispenserType);
        for(Point dispenserPosition:dispenserPositions) {
            ...
            if (DGM.getLength() < optimalDGM.getLength())
                optimalDGM = DGM;
        }
        return(optimalDGM);
    }
    ...
    public LinkedList<DSODispenserGoalMission> getOptimalAgents(){
        ...
        for(int taskType:PTask.getTypesNeeded()){
            ...
            if(PAgents!=null)
                PAgents.remove(mission.getAgent());
