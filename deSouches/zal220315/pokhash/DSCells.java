...
// bottom right corner
    ...
    public synchronized LinkedList<DSCell> getAllAt(Point point){
        ...
        if(PHashCells.containsKey(point))
            return((LinkedList<DSCell>)PHashCells.get(point).clone());
        else
            return(null);
    ...
    public synchronized LinkedList<DSCell> getAllType(int type){ // objekty daneho typu na vsech pozicich
        // Hash verze
